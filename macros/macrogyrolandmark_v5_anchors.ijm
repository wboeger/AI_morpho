// =====================================================================
// MACRO: Gyrodactylid Anchor Landmarking v5-anchors - Interactive Only
// =====================================================================
// Based on v5.5 (hook landmarking).
// v5-anchors changes:
//   - Landmarks adapted for ANCHORS (not hooks):
//       L1 = Point
//       L2 = External tip of superficial root
//       L3 = Distal-most base of deep root
//   - All other processing logic identical to v5.5
// =====================================================================
...
function sessionSetupDialog() {
    Dialog.create("Gyro-Landmark v5-anchors - Session Setup");

    Dialog.addMessage("=== DIRECTORY SETTINGS ===");
    Dialog.addString("Input directory:",
        "/Users/walterapboeger/Desktop/Gyromorphometry/AI_morpho/data/Images/18S/anchors", 60);
    Dialog.addString("Output directory:",
        "/Users/walterapboeger/Desktop/Gyromorphometry/AI_morpho/data/landmarks/18S_csv_100/anchors", 60);
...
function initializeSession() {
    ...
    print("\\Clear");
    print("=== GYRO-LANDMARK v5-anchors (Interactive) ===");
    print("Input:  " + dirIn);
    print("Output: " + dirOut);
...
function showCropGuide() {
    ...
    showMessage("Crop Guide",
        ...
        "Draw a rectangle AROUND the anchor structure.\n" +
...
function placeAnatomicalLandmarksInteractive() {
    ...
    //   L1 = Point
    //   L2 = External tip of superficial root
    //   L3 = Proximal-most base/shaft of deep root

    run("Select None");
    setTool("point");

    lX = newArray(3); lY = newArray(3);
    landmarkNames = newArray(
        "L1 (Point)",
        "L2 (External tip of superficial root)",
        "L3 (Proximal-most base of deep root)"
    );
    ...
    while (!allOK) {
        ...
        Dialog.addMessage(
            ...
            "  L1 (Point)                           at (" + d2s(lX[0], 1) + ",  " + d2s(lY[0], 1) + ")\n" +
            "  L2 (Ext. tip superficial root)       at (" + d2s(lX[1], 1) + ",  " + d2s(lY[1], 1) + ")\n" +
            "  L3 (Distal-most base of deep root)   at (" + d2s(lX[2], 1) + ",  " + d2s(lY[2], 1) + ")\n\n" +
...
function visualizeResults(finalX, finalY, nearL2idx, nearL3idx) {
    ...
    for (j = 0; j < nPoints; j++) {
        ...
        if      (j == 0)         Overlay.addSelection("cyan");    // L1: Point
        else if (j == nearL2idx) Overlay.addSelection("yellow");  // L2: Ext. tip superficial root
        else if (j == nearL3idx) Overlay.addSelection("magenta"); // L3: Distal-most base deep root
    ...
    while (currentStage != "DONE" && currentStage != "SKIP" &&
        ...
        if (currentStage == "CROP") {
            ...
            while (!cropOK) {
                ...
                if (firstAttempt) {
                    Dialog.create("Step 1: Define Anchor Region");
                    Dialog.addMessage("Draw a rectangle AROUND the entire anchor.");
                ...
                setTool("rectangle");
                waitForUser("STEP 1: Draw ROI Around Anchor",
                    "Draw rectangle around the anchor structure.\n\n" +
        ...
        // ────────────────────────────────────────── STAGE: ORIENT ──
            ...
            Dialog.addMessage("Does the anchor point to the RIGHT?\n(Choose Flip if yes.)");
            Dialog.addChoice("Action:",
        ...
        // ────────────────────────────────────────── STAGE: VERIFY ──
            ...
            while (!decideDone) {
                ...
                Dialog.addMessage("Points: " + nPointsTotal +
                                  ...
                                  "Colour key:\n" +
                                  "  CYAN    = L1 (Point)\n" +
                                  "  YELLOW  = L2 (Ext. tip superficial root)\n" +
                                  "  MAGENTA = L3 (Distal-most base deep root)\n" +
                                  "  GREEN   = equidistant semilandmarks");
...
function interactiveWandSelection() {
    ...
    while (!wandOK) {
        waitForUser("Wand Selection (tol=" + wandTol + ")",
            "Click on the anchor outline with the wand tool.\n" +
            "Then click OK.\n\n" +
...
function initialImageReview(fileName, filePath) {
    ...
    Dialog.addChoice("Rejection reason (if rejecting):",
        newArray("N/A", "Image too blurry", "Anchor damaged or broken",
                 "Wrong specimen / not an anchor", "Debris obscuring anchor",
